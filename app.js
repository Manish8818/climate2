...
app.put("/commentreply/:id", auth, async(req,res)=>{
  ...
  product.comments.forEach((item,indexOf)=>{
    if(item.user.toString()=== req.user._id.toString()){
      commentexist= true;
      commentIndex=indexOf;
  ...
  }else{ post.comments.push({
    user:req.user._id,
    comment:req.body.comment
  });
}
 await post.save();
 res.json({message:"comment on the post is updated successfully"})

})

app.post("/reply/:id", auth, async(req,res)=>{
  try{
  const post= await product.findOne(req.params.id)
  if(!post){
    res.json({message:"post not found"})
  }
  let commentdone=await post.comments.comment;
  let usercomment= await post.comments.user
  if(commentdone){
    product.reply.push({
      user:usercomment,
      comment:req.body.comment,
      alreadyexist:commentdone
      
    })
    await product.save()
    
  }
  else{ product.reply.push({
    user:undefined,
    comment:undefined
  })
    res.json({message:"pleasee comment on the post first"})
  }
}catch(err){
